...
function Kesfet({turnuvalar, T, git, ligKurAc, ligKurYetki, saltOkunur, yukleniyor, oturum, takimKurabilir, adminMi, onYeniSezon, onArsivle}){
  ...
  // Aynı oyuncu birden çok ligde olabilir → Keşfet Oyuncular listesinde TEK satır + tüm liglerin TOPLAMI (+ hangi takım/lig dökümü)
  const oyuncuBirlesik=useMemo(()=>{
    const m=new Map();
    tumOyuncular.forEach(o=>{ const key=o.id||o.player_id||o.ad; if(key==null) return;
      if(!m.has(key)) m.set(key,{...o, gol:0,asist:0,mac:0,dk:0,kurtaris:0,mvp:0, _ligler:[]});
      const g=m.get(key);
      g.gol+=(o.gol||0); g.asist+=(o.asist||0); g.mac+=(o.mac||0); g.dk+=(o.dk||0); g.kurtaris+=(o.kurtaris||0); g.mvp+=(o.mvp||0);
      if(!g.ovr && o.ovr) g.ovr=o.ovr; if(!g.foto && o.foto) g.foto=o.foto;
      if(o.turnuva) g._ligler.push({lig:o.turnuva, ligId:o.turnuvaId, takim:o.takimAd, gol:o.gol||0, asist:o.asist||0, mac:o.mac||0});
    });
    return [...m.values()];
  },[tumOyuncular]);
  // tüm maçlar
  ...
  const oyuncuFiltre=oyuncuBirlesik.filter(o=>(!q||o.ad.toLocaleLowerCase("tr").includes(q)) && (mevki==="hepsi"||o.poz===mevki));
  ...
  return <div className="fade-in main-area" style={{paddingBottom:90}}>
    ...
    {tab==="lig" && <div style={{padding:"12px 14px"}}>
      ...
      {gosterilecekLigler.filter(t=>(!q||((t.ad||"")+" "+(t.sehir||"")).toLocaleLowerCase("tr").includes(q)) && (t.durum||'aktif')!=='arsiv').sort(enYeniSirala).map(t=>{
        ...
        const fmtAd=t.sahaEsnek?"🎯 Hazırlık":({serbest:"Serbest",tek:"Tek Devre",cift:"Çift Devre",gruplu:"Gruplu",kupa:"🏆 Kupa"}[t.format]||"Serbest");
        const paylasimLink=t.slug?PAYLASIM_URL(t.slug):null;
          ...
          <div style={{display:"flex",gap:13,alignItems:"center",position:"relative",zIndex:1}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginTop:5}}>
                <span style={{fontSize:9.5,fontWeight:700,color:T.textSoft,background:T.bg2,border:"0.5px solid "+T.line,borderRadius:8,padding:"2px 7px"}}>⚽ {t.kisi} kişi</span>
                <span style={{fontSize:9.5,fontWeight:700,color:T.textSoft,background:T.bg2,border:"0.5px solid "+T.line,borderRadius:8,padding:"2px 7px"}}>{fmtAd}</span>
                {paylasimLink && <span onClick={e=>{ e.stopPropagation(); try{ navigator.clipboard && navigator.clipboard.writeText(paylasimLink); }catch(_){} try{ alert("🔗 Lig linki kopyalandı:\n"+paylasimLink); }catch(_){} }} className="tap" title="Lig paylaşım linkini kopyala" style={{fontSize:9.5,fontWeight:700,color:T.accent,background:T.accent+"14",border:"0.5px solid "+T.accent+"44",borderRadius:8,padding:"2px 8px",cursor:"pointer"}}>🔗 Link</span>}
              </div>
    ...
    {tab==="oyuncu" && <div style={{padding:"4px 14px"}}>
      ...
      {oyuncuSirali.slice(0,200).map((o,i)=>{
        ...
        const ligSay=(o._ligler?o._ligler.length:0), prim=o._ligler&&o._ligler[0];
        return <div key={oid} style={{background:T.bg1,borderRadius:11,marginBottom:5,border:"0.5px solid "+(acik?T.accent+"55":T.line),overflow:"hidden"}}>
          <div onClick={()=>oyToggle(oid)} className="tap satir-hover" style={{display:"flex",alignItems:"center",gap:11,padding:"9px 12px",cursor:"pointer",userSelect:"none",WebkitUserSelect:"none"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,color:T.text,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",display:"flex",alignItems:"center",gap:6}}>{o.ad}{ligSay>1 && <span style={{fontSize:8,fontWeight:800,color:T.accent,background:T.accent+"1e",border:"0.5px solid "+T.accent+"44",borderRadius:20,padding:"1px 6px",flexShrink:0}}>🏆 {ligSay} lig</span>}</div>
              <div style={{fontSize:9,color:T.textMut,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{o.poz||"Oyuncu"}{ligSay>1?" · "+ligSay+" ligde aktif":(prim?" · "+prim.takim+" · "+prim.lig:(o.takimAd?" · "+o.takimAd:""))}</div>
            </div>
          ...
          {acik && ligSay>1 && <div style={{padding:"2px 12px 6px"}}>
            <div style={{fontSize:8.5,letterSpacing:.5,fontWeight:800,color:T.textMut,margin:"4px 2px 4px"}}>🏆 LİG DAĞILIMI ({ligSay} lig)</div>
            {o._ligler.map((L,li)=><div key={li} style={{display:"flex",alignItems:"center",gap:7,fontSize:11,color:T.textSoft,padding:"5px 0",borderBottom:li<o._ligler.length-1?"0.5px solid "+T.line:"none"}}>
              <span style={{flex:1,minWidth:0,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>🏟️ {L.lig} · {L.takim}</span>
              <b style={{color:T.accent,fontFamily:T.fontDisplay}}>{L.gol}</b><span style={{fontSize:8,color:T.textMut}}>gol</span>
              <b style={{color:"#34D399",fontFamily:T.fontDisplay}}>{L.asist}</b><span style={{fontSize:8,color:T.textMut}}>as</span>
              <b style={{color:T.textSoft,fontFamily:T.fontDisplay}}>{L.mac}</b><span style={{fontSize:8,color:T.textMut}}>maç</span>
            </div>)}
          </div>}
...
function TurnuvaSayfa({turnuva, T, git, takipLig, ligTakip, yonetim, oturum, saltOkunur, onPaylas, onPaylasKaldir, ilkTab}){
  ...
  const [paylasUrl,setPaylasUrl]=useState(turnuva.slug?PAYLASIM_URL_TEMIZ(turnuva.slug):"");
  const [paylasYuk,setPaylasYuk]=useState(false);
  const [paylasHata,setPaylasHata]=useState("");
  const [kopyalandi,setKopyalandi]=useState(false);
  // Paylaş = TEMİZ LİNKİ paylaş (forzalig.com/<slug>). Artık herkese-açık kopya (snapshot) YOK → katalog karışmaz, giriş gerekir.
  const paylasAc=async()=>{
    setPaylasAcik(true); setPaylasHata("");
    let sl=turnuva.slug;
    if(!sl){                                   // güvenlik ağı: slug yoksa üret+kaydet (yeni ligler tetikleyiciyle zaten alır)
      setPaylasYuk(true);
      const s=slugUret(turnuva.ad||"lig");
      const r=await Db.ligSlugYaz(turnuva.id, s);
      setPaylasYuk(false);
      if(r&&r.ok){ turnuva.slug=s; sl=s; } else { setPaylasHata((r&&r.hata)||"Link oluşturulamadı."); return; }
    }
    setPaylasUrl(PAYLASIM_URL_TEMIZ(sl));
  };
  ...
  const [istAlt,setIstAlt]=useState("krallar");  // İstatistik çatısı alt-sekmesi: krallar/enler/grafik/kadro
  const [puanAlt,setPuanAlt]=useState("puan");   // Puan çatısı alt-sekmesi: puan/takimlar
  ...
  // Sadeleştirilmiş 4-sekme yapısı: Genel(=genel+akış+kurallar) · Puan(=puan+takımlar) · Fikstür · İstatistik(=krallar+enler+grafik+kadro)
  const sekmeler = (kupaMi
    ? [["kupa","🏆 Kupa"],["genel","Genel"],["ist","İstatistik"],["kurallar","📋 Kurallar"]]
    : [["genel","Genel"],["puan","Puan"],["fikstur","Fikstür"],["ist","İstatistik"],["kurallar","📋 Kurallar"]]
  ...
  return <div className="fade-in" style={{paddingBottom:90}}>
    ...
    {paylasAcik && <div onClick={()=>setPaylasAcik(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:1500,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div onClick={e=>e.stopPropagation()} className="fade-in" style={{width:"100%",maxWidth:460,background:T.bg1,borderRadius:"18px 18px 0 0",padding:"18px 18px calc(24px + env(safe-area-inset-bottom))",border:"0.5px solid "+T.line}}>
        ...
        <div style={{fontSize:12,color:T.textMut,marginBottom:16}}>Bu linki paylaş; açan kişi <b style={{color:T.accent}}>giriş yaptıktan sonra</b> ligini görür. (Herkese-açık kopya çıkmaz, katalog karışmaz.)</div>
        {paylasYuk ? <div style={{textAlign:"center",padding:20,color:T.textMut,fontSize:13}}>⚽ Link hazırlanıyor…</div> :
    ...
    {/* ===== İSTATİSTİK çatısı — alt-pill çubuğu (krallar/enler/grafik/kadro) ===== */}
    {tab==="ist" && <div style={{display:"flex",gap:6,overflowX:"auto",padding:"11px 12px 2px",WebkitOverflowScrolling:"touch"}}>
      {[["krallar","👑 Krallar"],["enler","🏆 Enler"],["grafik","📊 Grafikler"],["kadro","🏅 Haftanın 11'i"]].map(([k,l])=>
        <button key={k} onClick={()=>setIstAlt(k)} className="tap" style={{flexShrink:0,fontSize:12,fontWeight:700,padding:"8px 13px",borderRadius:999,whiteSpace:"nowrap",background:istAlt===k?T.gold:T.bg1,color:istAlt===k?"#2a1c00":T.textSoft,border:"0.5px solid "+(istAlt===k?T.gold:T.line)}}>{l}</button>
      )}
    </div>}

    {tab==="ist" && istAlt==="kadro" && (turnuva.takimlar.length>0 ? <KadroEkrani turnuva={turnuva} T={T} git={git}/> : <BosUyari T={T} metin="Henüz takım yok. Yönet sekmesinden takım ekle." />)}

    {/* ===== AKIŞ (Genel çatısı altında) ===== */}
    {tab==="genel" && !kupaMi && <div className="fade-in" style={{padding:"12px 14px",display:"flex",flexDirection:"column",gap:9}}>
      {/* Haftanın Kadrosu — teaser (İstatistik ▸ Haftanın 11'i) */}
      <div onClick={()=>{setTab("ist");setIstAlt("kadro");}} className="tap" style={{cursor:"pointer",background:"linear-gradient(135deg,"+T.gold+"22,"+T.bg1+")",borderRadius:12,padding:13,border:"0.5px solid "+T.gold+"55",display:"flex",alignItems:"center",gap:12}}>
    ...
    {/* ===== PUAN çatısı — Puan Durumu + Takımlar ===== */}
    {tab==="puan" && <div style={{display:"flex",gap:6,padding:"11px 12px 2px"}}>
      {[["puan","📊 Puan Durumu"],["takimlar","🛡️ Takımlar"]].map(([k,l])=>
        <button key={k} onClick={()=>setPuanAlt(k)} className="tap" style={{flex:1,fontSize:12,fontWeight:700,padding:"8px 6px",borderRadius:999,background:puanAlt===k?T.accent:T.bg1,color:puanAlt===k?"#fff":T.textSoft,border:"0.5px solid "+(puanAlt===k?T.accent:T.line)}}>{l}</button>
      )}
    </div>}
    {tab==="puan" && puanAlt==="puan" && (turnuva.takimlar.length>0 ? <PuanDurumu turnuva={turnuva} T={T} git={git}/> : <BosUyari T={T} metin="Henüz takım yok. Yönet sekmesinden takım ekle." />)}
    ...
    {/* ===== KRALLAR (İstatistik ▸ Krallar) ===== */}
    {tab==="ist" && istAlt==="krallar" && <div className="fade-in" style={{padding:"12px 14px"}}>
    ...
    {tab==="ist" && istAlt==="enler" && <div className="fade-in" style={{paddingTop:4}}>
      <TumEnler turnuvalar={[turnuva]} T={T} git={git} gomulu/>
    </div>}

    {/* ===== GRAFİKLER (İstatistik ▸ Grafikler) ===== */}
    {tab==="ist" && istAlt==="grafik" && <LigIstatistik turnuva={turnuva} ist={ist} T={T} git={git} onbirAltin={Motor.ideal11(turnuva,"altin")} pozKisa={pozKisa}/>}

    {tab==="puan" && puanAlt==="takimlar" && <div className="fade-in" style={{padding:"12px 14px"}}>
...
function OyuncuSayfa({oyuncu:o, T, takipOyuncu, oyuncuTakip, adminMod, git, turnuvalar, oturum, sahiplenme, onSahiplen, onTransfer, saltOkunur, adminMi}){
  ...
  // Bu oyuncu şu an HANGİ liglerde/takımlarda aktif — her birinde ne yaptı (çok lig desteği)
  const aktifLigler=useMemo(()=>{ const arr=[]; (turnuvalar||[]).forEach(t=>{ (t.takimlar||[]).forEach(tk=>{ const p=(tk.oyuncular||[]).find(x=>x.id===o.id); if(p) arr.push({lig:t.ad, ligId:t.id, takim:tk.ad, renk:tk.renk||T.accent, gol:p.gol||0, asist:p.asist||0, mac:p.mac||0}); }); }); return arr; },[o&&o.id, turnuvalar]);
  ...
  return <div className="fade-in" style={{paddingBottom:90}}>
    ...
    {/* ===== AKTİF LİGLER — oyuncu birden çok ligde oynuyorsa döküm + canlı toplam ===== */}
    {aktifLigler.length>1 && (()=>{ const tG=aktifLigler.reduce((s,x)=>s+x.gol,0), tA=aktifLigler.reduce((s,x)=>s+x.asist,0), tM=aktifLigler.reduce((s,x)=>s+x.mac,0);
      return <div style={{padding:"12px 14px 0"}}>
        <div style={{background:T.bg1,border:"0.5px solid "+T.line,borderRadius:14,padding:"13px 14px"}}>
          <div style={{fontSize:11.5,fontWeight:800,color:T.textSoft,marginBottom:3,display:"flex",alignItems:"center",gap:7}}>🏆 Aktif Ligler <span style={{fontSize:9,color:T.textMut,fontWeight:700,background:T.bg2,borderRadius:20,padding:"2px 8px"}}>{aktifLigler.length} lig</span></div>
          <div style={{fontSize:10,color:T.textMut,marginBottom:9}}>Bu oyuncu şu an hangi liglerde oynuyor, her birinde ne yaptı:</div>
          {aktifLigler.map((L,i)=><div key={i} onClick={()=>{ const t=(turnuvalar||[]).find(x=>x.id===L.ligId); if(t) git({sayfa:"turnuva",turnuva:t}); }} className="tap" style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<aktifLigler.length-1?"0.5px solid "+T.line:"none",cursor:"pointer"}}>
            <span style={{width:30,height:30,borderRadius:9,background:L.renk,display:"grid",placeItems:"center",fontSize:13,flexShrink:0}}>🏟️</span>
            <div style={{flex:1,minWidth:0}}><div style={{fontSize:12.5,fontWeight:700,color:T.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{L.lig}</div><div style={{fontSize:10,color:T.textMut,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{L.takim}</div></div>
            <div style={{display:"flex",gap:10,flexShrink:0}}>
              <div style={{textAlign:"center"}}><div style={{fontSize:13,fontWeight:800,color:T.accent,fontFamily:T.fontDisplay}}>{L.gol}</div><div style={{fontSize:7.5,color:T.textMut}}>GOL</div></div>
              <div style={{textAlign:"center"}}><div style={{fontSize:13,fontWeight:800,color:"#34D399",fontFamily:T.fontDisplay}}>{L.asist}</div><div style={{fontSize:7.5,color:T.textMut}}>AS</div></div>
              <div style={{textAlign:"center"}}><div style={{fontSize:13,fontWeight:800,color:T.textSoft,fontFamily:T.fontDisplay}}>{L.mac}</div><div style={{fontSize:7.5,color:T.textMut}}>MAÇ</div></div>
            </div>
          </div>)}
          <div style={{display:"flex",alignItems:"center",gap:10,marginTop:10,background:T.accent+"12",border:"0.5px solid "+T.accent+"44",borderRadius:11,padding:"10px 12px"}}>
            <span style={{flex:1,fontSize:11.5,fontWeight:800,color:T.accent}}>📊 Toplam (canlı)</span>
            <div style={{display:"flex",gap:13}}>
              <div style={{textAlign:"center"}}><div style={{fontSize:16,fontWeight:900,color:T.text,fontFamily:T.fontDisplay}}>{tG}</div><div style={{fontSize:7.5,color:T.textMut}}>GOL</div></div>
              <div style={{textAlign:"center"}}><div style={{fontSize:16,fontWeight:900,color:T.text,fontFamily:T.fontDisplay}}>{tA}</div><div style={{fontSize:7.5,color:T.textMut}}>ASİST</div></div>
              <div style={{textAlign:"center"}}><div style={{fontSize:16,fontWeight:900,color:T.text,fontFamily:T.fontDisplay}}>{tM}</div><div style={{fontSize:7.5,color:T.textMut}}>MAÇ</div></div>
            </div>
          </div>
        </div>
      </div>;
...
function Kuluplerim({T, oturum, git, turnuvalar, embedded, takimKurabilir, adminMi}){
  ...
  // BİRLEŞİK TAKIM LİSTESİ — lig takımları + lig-siz kulüpler, TEK liste, her takım BİR kez (kulup_id ile tekilleştir)
  const birlesikTakimlar=(()=>{
    const map=new Map();
    (turnuvalar||[]).forEach(t=>{ if(!t)return; (t.takimlar||[]).forEach(tk=>{ if(!tk)return; const key=String(tk.kulup_id||("t:"+tk.id)); if(map.has(key))return;
      map.set(key,{tip:'lig', tk, t, ad:tk.ad, renk:tk.renk, renk2:tk.renk2, logo:tk.logo, alt:'🏆 '+t.ad+(tk.oyuncular?(' · '+tk.oyuncular.length+' oyuncu'):''), benim:!!((oturum&&tk.yonetici_id===oturum.id)||adminMi), kulupId:tk.kulup_id||null}); }); });
    const kulupler=[...((liste||[]).map(c=>({...c,benim:true}))), ...((tumListe||[]).filter(c=>c&&!(liste||[]).some(x=>x.id===c.id)))];
    kulupler.forEach(c=>{ if(!c)return; const key=String(c.id); if(map.has(key))return; map.set(key,{tip:'kulup', c, ad:c.ad, renk:c.renk, renk2:c.renk2, logo:c.logo, alt:(c.benim?'⚽ Takımın':(c.katildi?'⚽ Oyuncu olarak katıldın':'Takım'))+((c.oyuncuSay!=null)?(' · '+c.oyuncuSay+' oyuncu'):''), benim:!!c.benim, kulupId:c.id}); });
    return [...map.values()];
  })();
  // Bir takıma tıkla → TAM takım sayfası (lig-siz kulüp için sanal turnuva; TakimSayfa aynen çalışır)
  const takimAc=async(item)=>{
    if(!item) return;
    if(item.tip==='lig'){ git({sayfa:'takim', takim:item.tk, turnuva:item.t}); return; }
    const c=item.c; let kad=[]; try{ kad=await Db.kulupKadro(c.id); }catch(e){}
    const oyuncular=(kad||[]).map(p=>({ id:p.player_id, player_id:p.player_id, ad:p.ad||'Oyuncu', poz:p.mevki||null, no:p.forma_no||null,
      ovr:p.ovr||60, pac:null,sho:null,pas:null,dri:null,def:null,phy:null, renk:c.renk||null, foto:p.foto||null, slug:null, kapak:null, sahip_user_id:null,
      gol:0,asist:0,mac:0,kurtaris:0,sari:0,kirmizi:0,mvp:0,dk:0, altin:0,gumus:0,forvet:0,ortasaha:0,defans:0,kaleci:0,centilmen:0,enerjik:0,macinGolu:0 }));
    const yon=c.sahip_user_id||(c.benim?(oturum&&oturum.id):null)||null;
    const takim={ id:'kulup-'+c.id, lisNo:null, ad:c.ad, slug:null, renk:c.renk||'#22E07A', renk2:c.renk2||null, logo:c.logo||null, kapak:c.kapak||null, yonetici_id:yon, td:c.td||null, kulup_id:c.id, oyuncular, grup:0, o:0,g:0,b:0,m:0,ag:0,yg:0,puan:0,sira:1,form:[] };
    const turnuva={ id:'kuluplig-'+c.id, ad:c.ad, renk:c.renk||'#22E07A', logo:c.logo||null, yonetici_id:yon, takimlar:[takim], maclar:[], format:'tek', grupSayi:0, sehir:null, ilce:null, kisi:7, seriId:null, sezonNo:1, slug:null, kapak:null, sponsorAd:null, sponsorEmoji:null, durum:'aktif', kurallarKilit:false, kurallar:'', iliskisel:false, _hafif:false, _kulupSanal:true };
    git({sayfa:'takim', takim, turnuva});
  };
  const KART2=(item)=>{ const rk=item.renk||T.accent; const selKey=item.kulupId||item.ad; const secli=secili2.has(selKey);
    const onCard=()=>{ if(secModu){ if(item.kulupId) secTogle(item.kulupId); } else takimAc(item); };
    return <div key={item.tip+':'+(item.kulupId||item.ad)} onClick={onCard} className="tap kart-hover" style={{display:"flex",alignItems:"center",gap:12,background:secli?T.danger+"14":T.bg1,border:(secli?"1px solid "+T.danger:"0.5px solid "+T.line),borderRadius:14,padding:13,marginBottom:9,cursor:"pointer"}}>
      {secModu && item.kulupId && <span style={{width:22,height:22,borderRadius:6,border:"2px solid "+(secli?T.danger:T.line),background:secli?T.danger:"transparent",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"#fff",flexShrink:0,fontWeight:800}}>{secli?"✓":""}</span>}
      <Logo renk={rk} ad={item.ad} logo={item.logo} renk2={item.renk2} boy={40}/>
      <div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:800,color:T.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{item.ad}</div><div style={{fontSize:11,color:T.textMut,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{item.alt}</div></div>
      {!secModu && item.benim && item.tip==='kulup' && <span onClick={e=>{e.stopPropagation(); kadroYukle(item.c);}} title="Yönet (davet · oyuncu ekle · lige katıl)" className="tap" style={{fontSize:15,color:T.textMut,padding:"4px 7px",flexShrink:0}}>⚙️</span>}
      <span style={{color:T.textMut,fontSize:20,flexShrink:0}}>{secModu?"":"›"}</span>
    </div>; };
  ...
  return <div style={{maxWidth:520,margin:"0 auto",padding:embedded?"4px 0 40px":"14px 14px 90px"}}>
    ...
    {!sec ? <>
      {liste===null ? <div style={{color:T.textMut,fontSize:13,padding:20,textAlign:"center"}}>Yükleniyor…</div> : <>
        {birlesikTakimlar.length>0 && <div style={{fontSize:11,color:T.textMut,fontWeight:700,letterSpacing:.5,margin:"2px 2px 8px",textTransform:"uppercase"}}>Takımlar · {birlesikTakimlar.length}</div>}
        {birlesikTakimlar.length===0 && !kur && <div style={{textAlign:"center",padding:"18px 10px 4px",color:T.textMut}}><div style={{fontSize:32,marginBottom:6}}>🛡️</div><div style={{fontSize:13,color:T.textSoft,lineHeight:1.6}}>{yetkili?"Henüz takım yok — aşağıdan kur.":"Henüz takım yok. Yetkili biri kurunca burada görünür."}</div></div>}
        {birlesikTakimlar.map(KART2)}
