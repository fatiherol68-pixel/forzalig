    ...
    {!sec ? <>
      {liste===null ? <div style={{color:T.textMut,fontSize:13,padding:20,textAlign:"center"}}>Yükleniyor…</div> : <>
        ...
        {!secModu && (()=>{ const seen=new Set(); const ligTak=[];
          ...
          // AYIKLA: ligde zaten görünen kulüpleri (kulup_id) tekrar "kulüp" olarak GÖSTERME → duplicate önlenir
          const ligKulupIds=new Set(ligTak.map(({tk})=>tk&&tk.kulup_id).filter(Boolean));
          const mineIds=new Set((liste||[]).map(c=>c.id)); const kulupTak=(tumListe||[]).filter(c=>c&&!mineIds.has(c.id)&&!ligKulupIds.has(c.id));
